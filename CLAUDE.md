# CLAUDE.md — claude-presence

Rust daemon (macOS) that aggregates live Claude Code activity into a single Discord Rich Presence.
...
Claude Code v2.1.181). No code is written without a corresponding task + acceptance criteria.
...
- Internal `~/.claude` schema reads go **only** through `src/claude/schema.rs` adapters, gated on
...
- Source of truth for the session set: `~/.claude/sessions/<PID>.json` + `kill(pid,0)` liveness.
...
- Discord application: name **"CC"** (Discord blocked "Claude"/"Claude Code"), **client_id = `1518007333324587168`**.
  Use this as the default `client_id` in config. The bold name on the card is "CC"; put "Claude Code"
  in `large_text` (tooltip).
- Assets (uploaded by the user in the Developer Portal → Rich Presence → Art Assets): `large_image`
  = the app picture; `small_image` = the Claude asterisk ("claude"). Images are OPTIONAL — the MVP
...
(`claude-opus-4-8`) unless overridden. Respect the `Depends`/`Files` graph for safe parallelism.
